...
export default function RequestedProperties() {
  ...
  if (!Array.isArray(requestedProperties) || requestedProperties.length === 0) {
    return (<div className="text-center text-gray-500 mt-20 p-6 bg-white h-full shadow-md rounded-lg">
      <h2 className="text-2xl font-bold text-gray-800 mb-4">No requested properties found.</h2>
      <button onClick={() => {
        navigate('/create-request-property')
      }} className='rounded-lg text-xl text-white  text-center bg-green-500 hover:bg-green-600 py-2 px-4 mt-10 lg:w-full max-w-[300px] mx-auto'>
  ...
  const handleViewProperty = (propertyId) => {
    navigate(`/requested-property-detail/${propertyId}`);
  }
      ...
      <div className="max-w-7xl mx-auto mt-20 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {requestedProperties.map((property) => (
            <PropertyCard key={property._id} property={property} onView={()=>handleViewProperty(property._id)} />
          ))}
        </div>
